...
class WandBLogger:
    ...
    def log(self, task: Task):
        ...
        for i in range(len(self.table)):
            attack_chain_prompt.append({
                "prompt": self.table["attack_prompt"][i], 
                "round": self.table["now_round"][i], 
                "action": self.table["action"][i]
            })
            attack_chain_response.append({
                "response": self.table["target_response"][i], 
                "round": self.table["now_round"][i], 
                "action": self.table["action"][i]
            })
        ...
        try:
            ...
            print(f"Error in logging data to wandb: {e}")
...
class LocalLogger:
    """Local file system logger for experiment tracking."""

    def __init__(self, args, init_chain, project_name="MR-Attacks"):
        logger_name = f"{args.target_model}_{self.sanitize_name(args.target)}_{args.batch_id}"
